...
async function syncSheets() { const selectedSport = one('[data-reg-sport].active')?.dataset.regSport; const sport = selectedSport === 'cricket' ? 'cricket' : 'football'; try { await api(`/admin/registrations/sync/${sport}`, { method:'POST' }); notify(`${sport} Google Sheet synced.`); } catch(error) { notify(error.message, false); } }
